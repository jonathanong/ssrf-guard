...
// Use unknown to bypass strict type-checking differences between undici's Response and Node's Response types.
function makeResponse(status: number, headers: Record<string, string> = {}): unknown {
...
describe("safeFetch (mocked)", () => {
  ...
  it("strips sensitive headers on cross-origin redirects", async () => {
    ...
    const secondHeaders = new Headers(secondCallInit?.headers as never);
  ...
  it.each([303, 302, 301])(
    "changes method to GET and drops body on %i redirect from POST",
    async (status) => {
      mockRedirect(status, "https://example.com/final");

      await safeFetch("https://example.com/", {
        method: "POST",
        body: "secret-body" as never,
      });
